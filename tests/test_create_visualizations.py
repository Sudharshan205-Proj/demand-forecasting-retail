...
def metadata_columns(block: ET.Element) -> list[tuple[str, int]]:
    """Return ``(name, ordinal)`` for the column records of one block."""
    return [
        (record.findtext("remote-name"), int(record.findtext("ordinal")))
        for record in block.findall("metadata-record")
        if record.get("class") == "column"
    ]


def test_workbook_hides_only_the_fields_no_sheet_reads() -> None:
    """The five unused fields are hidden, which is why the extracts omit them.

    Hiding is a deliberate data-source decision, not drift: any sheet wanting
    one of these fields would have to unhide it, and that would change what the
    extract metadata below is allowed to contain.
    """
    assert hidden_columns(WORKBOOK.read_text(encoding="utf-8")) == {
        "order",
        "season_length",
        "p90_daily_demand",
        "p95_daily_demand",
...
def test_schema_sync_keeps_hidden_fields_out_of_the_extract(
    tmp_path: Path,
) -> None:
    """A hidden field is never re-introduced into the extract's metadata."""
    workbook, analysis = copy_workbook_and_sources(tmp_path)

    assert sync_workbook(workbook, analysis).changed is False

    sources = datasources(ET.parse(workbook).getroot())
    hidden = {
        "inventory_variability_summary.csv": {
            "p90_daily_demand",
            "p95_daily_demand",
        ...
        "tuned_validation_results.csv": {"season_length", "order"},
    }

    for filename, fields in hidden.items():
        blocks = sources[filename].findall(".//metadata-records")
        assert len(blocks) == 2

        live = metadata_columns(blocks[0])
        extract = metadata_columns(blocks[1])

        # The live relation carries every CSV column, with CSV ordinals.
        assert fields <= {name for name, _ in live}
        assert [ordinal for _, ordinal in live] == list(range(len(live)))

        # The extract omits the hidden fields and numbers its own columns.
        assert not fields & {name for name, _ in extract}
        assert [ordinal for _, ordinal in extract] == list(range(len(extract)))
