...
# ---------------------------------------------------------------------------
# aggregate_sales correctness across chunk boundaries.
#
# None of the tests above call aggregate_sales, so a regression in its
# unique-count, mean or Kahan-summation logic would pass the whole suite.
# ---------------------------------------------------------------------------


def _write_sales_csv(tmp_path):
    """Six rows arranged so that, with chunksize=2, every (date, store, item)
    group has members split across more than one chunk. A dataset that fits
    in a single chunk would not exercise the cross-chunk accumulation logic.
    """
    rows = [
