...
evidence the project already produced. It consumes nine compact artifacts rather
than reloading the 1.28 GB integrated dataset:
...
(`sys.nframe() == 0L`), so the R Markdown report can source it and call it itself.
...
2. `dplyr::inner_join()` to align the demand and variability summaries, and to
   align the published insights with R's independently computed values.
...
The workflow also verifies the summary's internal arithmetic, so a demand summary
that does not equal its own total over its own day count fails the run instead of
being reported.
...
A higher CV indicates greater demand variability relative to the store's average
demand. R verifies that the demand summary and the variability summary publish
the same CV, and that the CV equals σ ÷ mean in both.
...
- z = standard-normal quantile of the service level, checked against `qnorm()`;
...
demand (`model_daily_demand + bias`), and the workflow asserts that identity so
the family cannot be described as a forward forecast.
...
forecast-error summary. `yardstick` reports MAPE as a percentage, matching the
Phase 12 field.
...
The report's final chunk stops the knit if any quality check failed, and figures
are referenced relative to the report so it can be knitted from the repository
root.
...
Observed demand statistics describe historical training-period behaviour on the
densified series Phases 11–13 use.
