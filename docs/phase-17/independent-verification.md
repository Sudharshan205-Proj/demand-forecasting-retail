# Independent Verification Report — Retail Demand Forecasting (Phases 0–17)

Independent, evidence-based audit, validation and from-scratch re-execution performed 2026-09-19. Every statement below is tied to an executed command, a test result, an artifact inspection or a checksum.

*Provenance:* this is the independent verification record, produced after Phase 17 closed. Its supporting working files — the per-command stdout/stderr captures, the audit's working state and log files (`audit-state.md`, `phase-status.md`, `command-log.md`, `change-log.md`, `execution-log.md`, `performance-log.md`, `regression-log.md`, `artifact-validation.md`, `link-audit.md`, `file-inventory.md`, `documentation-audit.md`) and the per-file documentation review — were removed from the repository in the 2026-09-19 documentation cleanup, because they recorded the audit's own working process rather than the project. Nothing below depends on them: every claim is re-derivable by running the commands in `docs/reproducibility-runbook.md`, and their findings are consolidated in `docs/phase-17/audit-findings.md`.
...
- **No project file was modified by the audit.** A follow-up per-file review of all 220 files under `docs/` (2026-09-19) found documentation drift that this report originally under-reported: four stale per-phase test counts, an artifact byte-size error, Phase 9's full-suite total, missing master-record entries and two stale Git/structural figures. All are now corrected — see `audit-findings.md` DOC-01/DOC-02/DOC-06–DOC-12.
- The single baseline warning was **ENV-01**, the pandas `to_datetime` format-inference UserWarning; it was subsequently resolved on 2026-09-19 with `format="mixed"`, so the suite now runs warning-free.
...
**Verified by reconstruction.** Running every documented command from Phase 0 onward reproduced the documented results, with processed datasets byte-identical and raw data untouched. The runbook (`docs/reproducibility-runbook.md`) documents prerequisites, dataset acquisition, commands, expected outputs/times, verification and common issues. Residual caveats: generated data is Git-excluded (the committed deploy bundle covers application startup); the two reference documents are untracked by owner decision; and hosted deployment requires owner authorisation.

## Link Audit

Four documented external links, all HTTP 200 on 2026-09-19: the Tableau Public dashboard (and its canonical author URL) and the Kaggle dataset source/download endpoints. No dead or undocumented links (the audit's link-check record; see *Provenance*).
...
None in code, tests, configuration or data. The documentation inaccuracies found by the follow-up review — four stale test counts, one artifact byte size, Phase 9's full-suite total, four missing master-record entries, the tracked-file count and the HEAD record — are listed in `audit-findings.md` and are all corrected.
...
No code changed, so no change-driven regression was required. Reconstruction regression is green: each downstream phase reconciled to its upstream output, and both full-suite runs passed 530/530 (the audit's regression log; see *Provenance*).
...
**None to project code, tests, configuration or data during the audit itself.** The audit's own additions — the working files listed under *Provenance* — were never importable from project code and could not affect it (`testpaths = ["tests"]`). The final 530-test run post-dated those additions.

One post-audit code change was applied on 2026-09-19 to resolve **ENV-01**: `scripts/clean_retail_data.py` now calls `pd.to_datetime(..., errors="coerce", format="mixed")`, which removes the pandas `to_datetime` UserWarning without changing parsed values.
...
7. Documentation drift (DOC-01/02, DOC-06–DOC-12) — **all corrected 2026-09-19** (see `audit-findings.md`).
...
18/18 phases executed with exit 0; 6/6 quality gates fully passed; application HTTP 200; complete stdout/stderr captured per phase during the audit (see *Provenance*). Raw data unmodified; 120/124 artifacts byte-identical, 4 timestamp-only.
...
> The "1 warning" in both audit runs was the ENV-01 pandas `to_datetime` UserWarning. It was resolved on 2026-09-19 by passing `format="mixed"` in `scripts/clean_retail_data.py`; the suite now runs warning-free (verified by running the Phase 5 module with `-W error::UserWarning`).
