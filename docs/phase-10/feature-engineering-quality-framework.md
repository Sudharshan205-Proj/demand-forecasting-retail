...
The feature-engineering pipeline must produce a reproducible, structurally valid,
...
Evidenced by `calendar_features_present` (7 of 7) and
`historical_features_present` (9 of 9). A missing feature reduces the reported
count and fails the check.
...
Evidenced by `duplicate_date_item_store_keys` (0).

### Chronology

Verify that each item-store series is ordered chronologically.

Evidenced by `chronological_within_item_store_series` (True).

### Target Integrity

Verify that feature engineering does not alter the physical demand target.

Evidenced by `target_preserved_vs_input` (True, compared row by row against the
Phase 9 source) and `target_quantity_missing` (0).
...
`split_preserved_vs_input`, all True.

### Reconciliation

Compare the quantity total before and after feature engineering.

Evidenced by `quantity_total_reconciled` (41,949,529.910 on both sides),
`input_output_rows_match` (7,431,026) and `input_output_keys_match`.

### Reproducibility

The same input and implementation produce the same feature values.

The transformation is deterministic and the output dataset is stable across
re-executions (887,995,450 bytes, SHA-256
...
A separate `feature_engineering_feature_summary.csv` records the completeness of
all 16 features.
