...
def aggregate_price_history() -> pd.DataFrame:
    ...
    Price history holds price-change events rather than a complete daily
    price series, so Phase 6 records the event and leaves forward-filling to
    later feature engineering, where it can respect chronological order.
    When several events share one key, ``last`` resolves the tie by file
    order -- the raw files are not chronologically sorted, so the
    representative price and code are file-order dependent. That is a
    tie-break within a single key (the key already contains the date), not a
