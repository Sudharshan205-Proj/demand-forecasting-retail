...
(`source_rows - prepared_rows`, verified 0), which would expose any
cross-chunk merge of repeated keys.
...
within-chunk duplicate scan is recorded
(`within_chunk_duplicate_keys`, verified 0). Because chunks are read
sequentially, a duplicate key split across a chunk boundary would not be
detected by that scan — the same chunk-local limitation documented in Phase 5.
It cannot change the result: the final groupby merges any repeated key and the
recorded row reduction exposes the merge. A non-zero duplicate count stops the
workflow.
...
calendar dates, counted as `difference - 1`.

Verified: 58,022 item-store series, of which 55,122 contain at least one gap,
...
which the previous implementation did not honour.

Verified boundaries for the 761-date history: train ends 2024-02-10 (532
