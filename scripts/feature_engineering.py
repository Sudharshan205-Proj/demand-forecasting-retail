...
The script turns the Phase 9 time-series dataset into a feature dataset for
later forecasting and machine-learning phases. The feature groups are
calendar features, historical demand lags, shifted rolling demand statistics
and series age, all built against the ``quantity`` target; demand-derived
features use prior observations only, so the current target never enters its
own lag or rolling feature. The Phase 9 chronological split labels are
preserved, and the feature matrix is reconciled against the Phase 9 input on
rows, keys, target, split labels and the quantity total. A per-feature
completeness artifact records the missing-value structure of the lag and
rolling features at each series start, and an explicit ISO date format keeps
the calendar features deterministic -- an invalid or ambiguous date raises
instead of being inferred element by element.

One documented limitation: lag and rolling windows operate over previous
*observed records*, and because Phase 9 found substantial intermediate-date
gaps, ``lag_7`` is the seventh previous observed record rather than
necessarily the observation seven calendar days earlier.
"""
