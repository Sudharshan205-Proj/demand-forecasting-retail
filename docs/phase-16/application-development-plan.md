...
Deliver a lightweight decision-support application that exposes the validated
retail demand forecasting and inventory-analysis results produced by the previous
project phases.
...
It does not retrain forecasting models during application use.

## Inputs

The application reads seven compact CSV artifacts. Their names are declared once,
in `app/config.py`, and resolved in this order:
...
The large processed and feature-engineered datasets are intentionally not loaded
by the Streamlit interface.
...
This separation supports maintainability and testing. Every Streamlit call lives
inside `main()`, so the pure helpers can be imported and tested without a running
Streamlit runtime.
