...
The script aggregates transaction-level integrated data to the
date-item-store forecasting grain while preserving chronological order, and
it does not create lag features or train models. The source is read once, in
chunks, with source row, quantity and date statistics accumulated during the
same pass. Missing observations stay missing: only observed date-item-store
records are kept and a gap is never filled with zero demand. The
chronological train/validation/test partitions are computed from unique
dates only, so no future observation can enter an earlier partition. Dates
are parsed with an explicit ISO format so the chunked pass is deterministic
-- an unparsable date becomes missing and fails validation instead of being
inferred element by element.
"""
...
def aggregate_daily(
    ...
    date-item-store key; source verification statistics are accumulated in
    the same pass, so the source is never loaded as a whole. Returns the
    aggregated frame and a ``source_stats`` dict recording the source row
    count, quantity, date range and within-chunk duplicate count.
    """
