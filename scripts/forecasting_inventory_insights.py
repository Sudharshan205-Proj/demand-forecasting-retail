...
Two scenario families are produced:

* a historical-variability family, whose uncertainty input is the observed
  training-period daily demand standard deviation; and
* a forecast-driven family, whose uncertainty input is the realised error of
  the Phase 12 selected model on the validation period, with the measured
  bias applied as a level correction.

The Phase 12 test period is never read. Historical demand statistics are
computed on the training period only, and the forecast evidence comes from
the stored Phase 12 validation predictions.

The demand series is densified onto a complete daily calendar with the same
construction Phase 11 and Phase 12 use, so all three phases describe an
identical store-day series. Densification is measured and recorded rather
than assumed.
"""
...
def load_training_demand() -> tuple[pd.DataFrame, SourceStats]:
    ...
    aggregate rather than to the 7.4-million-row source.

    Densification mirrors Phase 11's ``to_regular_daily_series`` exactly:
    each store's observed training range is reindexed onto a complete daily
    calendar with absent days filled at zero. An absent date at this grain
    means no quantity was recorded for that store that day.

    Returns the densified frame, which carries an ``is_synthetic`` flag
    identifying zero-filled days, and the source totals used for
    reconciliation.
    """
    stats = SourceStats()
...
def calculate_forecast_scenarios(
    ...
    for error in errors.itertuples(index=False):
        ...
        # demand that was actually realised over the validation period. This
        # is deliberately NOT presented as a forecast of future demand: the
        # only genuinely unseen period is the test set, which Phase 12 and
        # Phase 13 both leave reserved. Both levels are carried so the
...
def build_findings_text(
    ...
    report cannot contradict the artifacts it describes. The previous
    version hardcoded "Store 4 has no Phase 12 tuned forecasting
    configuration", which survived re-execution and contradicted the phase's
    own data.
    """
    highest_demand = demand_summary.loc[
        demand_summary["mean_daily_demand"].idxmax()
...
def build_quality_report(
    ...
    # validation mean, so it is a recovered demand level rather than a
    # forward forecast. Asserting the identity keeps that interpretation
    # honest in both the artifacts and the documentation.
    identity_match = bool(
        np.allclose(
            level_reference["bias_adjusted_daily_demand"].to_numpy(
                dtype=float
