...
- The Phase 10 feature-engineered input is reconciled at the store-day
  grain (rows, keys, total quantity) before any model is fitted.
- Every store series is densified onto a complete daily calendar, and
  the densification is measured rather than assumed.
- Naive and seasonal-naive forecasts are verified to reproduce their
  documented definitions, and ARIMA's order is recorded explicitly.
- RMSE and MAPE are recomputed from the stored predictions and
  reconciled against the reported metrics.
- No observation at or after ``TEST_START`` is used for training,
...
def create_summary(
    ...
    pooled_records = [
        {
            "model": model,
            "pooled_observations": int(len(group)),
            "pooled_rmse": rmse(
...
def write_findings(
    ...
    for row in results.sort_values(["store_id", "model"]).itertuples(
        ...
        lines.append(
            f"{str(row.store_id):>5}  {row.model:<15} "
