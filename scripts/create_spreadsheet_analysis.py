...
def _kahan_add(total: float, compensation: float, value: float) -> tuple[float, float]:
    ...
    Plain `total += value`, repeated once per chunk for every group, is
    ordinary floating-point addition: each addition can lose a few low-order
    bits and the rounding error compounds as more chunks are added, worse
    when a group's per-chunk sums differ in magnitude. Kahan summation keeps
    a running "compensation" term that captures the bits lost on each
    addition and feeds them back on the next one, so the accumulated error
    stays around one machine epsilon however many chunks are summed.
    """
...
def format_worksheet(worksheet) -> None:
    ...
    Deliberately does not set ``worksheet.auto_filter.ref``: sheets that get
    an Excel Table already carry their own AutoFilter, and a second
    worksheet-level AutoFilter over the same range is invalid OOXML -- Excel
    strips the Table/AutoFilter features on open. Sheets that do not get a
    table should set ``worksheet.auto_filter.ref = worksheet.dimensions``
...
def create_workbook(
    ...
    # The five aggregate formulas reference the DailySalesTable table
    # rather than a fixed cell range, so they span every daily row and
    # cannot silently miss the table's true maximum or minimum if the
    # row count changes.
    formula_rows = [
        ["Spreadsheet Technique", "Example / Result"],
        [
            "SUM",
            "=SUM(DailySalesTable[total_quantity])",
        ],
        [
            "AVERAGE",
            "=AVERAGE(DailySalesTable[average_price])",
        ],
        [
            "MAX",
            "=MAX(DailySalesTable[total_quantity])",
        ],
        [
            "MIN",
            "=MIN(DailySalesTable[total_quantity])",
        ],
        [
            "SUMPRODUCT",
            ("=SUMPRODUCT(DailySalesTable[total_quantity],"
            "DailySalesTable[average_price])"),
        ],
        [
