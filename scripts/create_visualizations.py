...
The figures are built from compact analytical outputs produced by earlier
phases: no model is retrained and the test period is never used for
selection or tuning. The project root is discovered by walking up from the
working directory for ``pyproject.toml`` and can be overridden -- together
with the input, output and report directories -- through
``VISUALIZATION_PROJECT_ROOT``, ``VISUALIZATION_INPUT_DIR``,
``VISUALIZATION_OUTPUT_DIR`` and ``VISUALIZATION_REPORT_DIR``, which is how
the automated tests run the workflow into a temporary directory. Validation
results are recorded in a machine-readable quality report
(``visualization_quality_report.csv``) that gates the run: a failed check
means no "completed" message, a non-zero exit code and withheld figures.
Each generated figure is recorded in ``visualization_manifest.csv`` with its
byte size and SHA-256 digest, under a project-relative path so the manifest
...
def manifest_path(figure: Path, root: Path | None) -> str:
    """Return the figure path relative to the project root when possible.

    The manifest records a repo-relative path so it stays portable. A run
    that redirects its output outside the project root (the automated tests)
    cannot be made relative to it, so the absolute posix path is kept there.
    """
    if root is not None:
        try:
            return figure.resolve().relative_to(root.resolve()).as_posix()
        except ValueError:
            pass

    return figure.as_posix()
...
def write_manifest(
    ...
    rows = [
        {
            "figure": figure.name,
            "path": manifest_path(figure, root),
...
def run_workflow(
    *,
    ...
    manifest_file = write_manifest(
        figures, sources, paths.report_dir, root=paths.root
    )
