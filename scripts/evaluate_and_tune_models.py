...
training period, tunes seasonal-naive, ARIMA and feature-based
configurations, evaluates the selected configurations on the untouched
validation period, and writes reproducible evaluation artifacts.

The final test period is intentionally excluded from model selection and
tuning.

Audit-hardened guarantees (Phase 17 re-audit):

- Every store is tuned: the cross-validation design adapts its fold count
  to the history a store actually has, instead of silently dropping short
  series.
- Configurations are passed as typed objects. No configuration string is
  ever parsed back into an order or a season length.
- Predictions for every fold and for the validation evaluation are stored,
  so every reported metric can be recomputed independently.
- The Phase 10 feature families (lag, rolling and calendar features) are
  now consumed by a feature-based candidate model, computed at the same
  store-day forecasting grain as the classical models.
- A machine-readable quality report verifies the input reconciliation,
  split boundaries, fold structure, leakage controls, selection rule and
  metric reproduction, and the run is gated on every check passing.
"""
...
# statsmodels/sklearn for malformed input), NaNs reaching the ARIMA fitter
...
# A fold must be able to fit the longest seasonal window (28 days), so the
# shortest series that can be tuned at all still has a usable initial
# training sample.
