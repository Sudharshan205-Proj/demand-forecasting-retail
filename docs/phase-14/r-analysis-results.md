...
| RStudio | 2026.8.2.200 | Recorded in `docs/phase-0/environment.md`; the IDE run is recorded in [`phase-14-checklist.md`](phase-14-checklist.md) |
...
| Command (from `r/`) | `Rscript r_analysis.R` — resolves the same root |
| Result | Exit 0, `Phase 14 R analysis completed successfully.` |
| IDE run | Executed in RStudio on 2026-09-19 — `r/r_analysis.R` sourced, `r/r_analysis_report.Rmd` knitted; same 91 of 91 result |
| Quality checks | 91 of 91 passing |
| Inputs consumed | 9 Phase 13/Phase 12 artifacts |
| Outputs written | 12 files (11 data/report files + the knitted HTML report) and 4 figures |
...
All outputs are generated artifacts and are excluded from Git under the project's
generated-artifact policy (`data/analysis/*`).
...
Densified training-period demand, exactly as Phase 13 computed it on the series
Phases 11–12 fitted.
...
- Highest relative variability: **Store 4** (CV 0.380098), whose history is the
  shortest of the four stores.
...
Both Phase 13 scenario families are analysed. Reorder points at the 14-day / 95 %
planning point:
...
forecast-error family sizes them from the selected model's realised validation
error and its bias-corrected validation demand level. Neither is an operational
requirement.
...
predictions artifact. Mean absolute error was not published by Phase 12, so it is
R-derived evidence rather than a reconciliation.
...
Totals: 1,656 densified store-days, 1,655 observed, one zero-filled (Store 3,
2022-10-16), matching Phase 11's and Phase 13's measurement.
...
| Output verification | 12 files written (11 data/report files + the knitted HTML report) and 4 figures written and non-empty, test period excluded | pass |
...
The report renders two further figures inline, demonstrating `aes()` mappings,
layered geoms, `facet_wrap()` and labels, and embeds the exported figures.
...
- `R_ANALYSIS_PROJECT_ROOT`, `R_ANALYSIS_INPUT_DIR` and `R_ANALYSIS_OUTPUT_DIR`
  override the resolved paths, which is how the tests run the complete workflow
...
- The dataset provides no supplier lead times, service-level requirements,
  holding costs, ordering costs, stockout costs, current stock, purchase orders
  or warehouse constraints.
- Store 4 has 60 training days and one cross-validation fold; its variability and
  forecast-error estimates rest on fewer observations.
