...
report records `quantity_numeric_and_complete` (True).
...
Evidenced by `duplicate_date_item_store_keys` (0) and
`within_chunk_duplicate_source_keys` (0). The row-reduction metric
`prepared_rows_do_not_exceed_source_rows` (7,431,026 ≤ 7,431,026) exposes any
cross-chunk merge that a chunk-local scan cannot see.
...
Evidenced by `chronological_within_item_store_series` (True) and
...
Evidenced by `quantity_numeric_and_complete` (True) and
`source_quantity_equals_prepared_quantity` (41,949,529.910 on both sides).
...
Evidenced by `maximum_gap_within_observed_span` (757 ≤ 760 days) and
`missing_intermediate_days_within_span` (756 ≤ 760 days). Every gap is bounded
by the observed series span, and the overall totals are recorded in the summary
and findings.
...
`split_rows_reconcile_with_prepared`, all True. The split row counts
...
modification time before and after the run (True).
