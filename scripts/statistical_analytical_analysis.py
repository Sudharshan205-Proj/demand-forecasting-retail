...
statistical summaries for demand-forecasting decisions. The analysis is
descriptive and inferential: it does not train forecasting models and does
not create forecasting features. Every reported statistic is computed from
the complete dataset -- chunk-level centered sums are merged with stable
parallel-accumulation formulas, so nothing is computed from a sample --
except the price/demand scatter figure, which uses a deterministic
systematic sample (every SCATTER_SAMPLE_STEP-th eligible record) so it spans
the full date range instead of a contiguous block of the file.
"""
